                            ...
                            <table class="table datatable" id="pegawai">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $item->alamat }}</td>
                                        <td>
                                            @if ($item->lahans->count() > 0)
                                                <a href="{{ route('data-pemilik.show', $item->id) }}" class="btn btn-info btn-sm"> {{ $item->lahans->count() }} lahan </a>
                                            @else
                                                -
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('data-pemilik.edit', $item->id) }}" class="btn btn-primary btn-sm">
                                               ...
                                               <div class="modal-dialog">
                                                   <div class="modal-content">
                                                       ...
                                                       <div class="modal-body">
                                                           Apakah Anda yakin ingin menghapus data dari <strong>{{ $item->nama_pemilik }}</strong>?
                                                       </div>
                                                       <div class="modal-footer">
                                                           <form action="{{ route('data-pemilik.destroy', $item->id) }}" method="POST">
                                                               @csrf
                                                               @method('DELETE')
